...
func SaveReflection(c *fiber.Ctx) error {
	...
	// Basic validation
	if reflection.UserID == "" || reflection.SessionID == "" || reflection.Text == "" {
		return c.Status(400).JSON(fiber.Map{"error": "Missing required fields"})
	}

	// Generate unique ID (e.g. sessID-userID)
...
func GetInsights(c *fiber.Ctx) error {
	...
	// Get sessions where user is partnerA or partnerB
	...
	// Get reflections written by this user
